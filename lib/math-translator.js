...
var MATH_RULES_LOCALES = {
  // Number formats
  THOUSAND_SEP_AS_THIN_SPACE: ['az', 'bg', 'cs', 'de', 'fr', 'hu', 'it', 'ka', 'km', 'ky', 'lt', 'lv', 'nb', 'nl', 'pl', 'pt-pt', 'ro', 'sq', 'sv', 'uk'],
  THOUSAND_SEP_AS_DOT: ['da', 'el', 'id', 'is', 'mk', 'pt', 'rw', 'sl', 'sr', 'tr', 'vi'],
  NO_THOUSAND_SEP: ['hy', 'kk', 'ko', 'ru', 'zh-hans'],
  DECIMAL_COMMA: ['az', 'bg', 'cs', 'da', 'de', 'el', 'fr', 'hu', 'hy', 'id', 'is', 'it', 'ka', 'kk', 'ky', 'lt', 'lv', 'mk', 'nb', 'nl', 'pl', 'pt', 'pt-pt', 'ro', 'ru', 'rw', 'sl', 'sq', 'sr', 'sv', 'tr', 'vi'],
  ...
  OVERLINE_AS_PARENS: ['az', 'bg', 'hy', 'ka', 'kk', 'ky', 'lt', 'lv', 'pl', 'pt-pt', 'ro', 'ru', 'vi'],
  ...
  TIMES_AS_CDOT: ['az', 'bg', 'cs', 'da', 'de', 'hu', 'hy', 'lt', 'lv', 'nb', 'pl', 'ro', 'sr', 'sv'],
  CDOT_AS_TIMES: ['fr', 'ps', 'pt-pt'],
  DIV_AS_COLON: ['az', 'bg', 'cs', 'da', 'de', 'hu', 'hy', 'it', 'ky', 'lt', 'lv', 'nb', 'pl', 'pt-pt', 'ro', 'ru', 'sv', 'uk'],
  // Trig functions
  SIN_AS_SEN: ['it', 'pt', 'pt-pt'],
  TAN_AS_TG: ['az', 'bg', 'cs', 'hu', 'hy', 'kk', 'km', 'ky', 'lt', 'lv', 'pl', 'pt', 'pt-pt', 'ro', 'ru'],
  COT_AS_COTG: ['cs', 'pt', 'pt-pt'],
  COT_AS_CTG: ['az', 'bg', 'hu', 'hy', 'kk', 'km', 'ky', 'lt', 'lv', 'pl', 'ro', 'ru'],
  CSC_AS_COSEC: ['as', 'az', 'bg', 'bn', 'cs', 'gu', 'hi', 'id', 'ja', 'kn', 'ky', 'lt', 'lv', 'mr', 'my', 'pa', 'pl', 'ro', 'ru', 'sv', 'ta', 'te', 'tr', 'uk'],
  ...
  // m\angle ABC -> \angle ABC
  ANGLE_MEASURE_WITHOUT_M: ['am', 'as', 'az', 'bg', 'bn', 'da', 'de', 'el', 'fa', 'fa-af', 'fv', 'gu', 'he', 'hi', 'hu', 'hy', 'id', 'is', 'it', 'ja', 'ka', 'kk', 'km', 'kn', 'ko', 'ky', 'lt', 'mk', 'mn', 'mr', 'my', 'nb', 'nl', 'pa', 'pl', 'ps', 'ru', 'rw', 'sl', 'so', 'sq', 'sr', 'sv', 'sw', 'ta', 'te', 'uk', 'ur', 'uz', 'xh', 'zh-hans', 'zh-hant', 'zu'],
...
function translateMath(math, template, lang) {
  ...
  math = translateNumbers(math, lang); // This one needs to be last

  math = translateNumerals(math, lang); // Special notation for numbers not handled above
...
function translateNumbers(math, lang) {
  ...
  var mathTranslations = [// IMPORTANT NOTE: This MUST be the first regex
...
function translateMathOperators(math, lang) {
  var mathTranslations = [// BINARY OPERATORS
  ...
  }, // Let's deal with angle measures first!
  {
    ...
    regex: /(^|[^a-zA-Z])m\s*\\angle\s+([a-zA-Z]|[A-Z]{3})(?![a-zA-Z])/g,
    replace: '$1|\\angle $2|'
  }, // m\\angle ABC -> A\hat{B}C
  {
    langs: MATH_RULES_LOCALES.ANGLE_MEASURE_AS_HAT,
    regex: /(^|[^a-zA-Z])m\s*\\angle\s+([A-Z])([A-Z])([A-Z])(?![a-zA-Z])/g,
    replace: '$1$2\\hat{$3}$4'
  }, // m\\angle ABC -> m(A\hat{B}C)
  {
...
function normalizeTranslatedMath(math, lang) {
  ...
  var mathNormalizations = [// Strip superfluous curly braces around \\,
...
function maybeTranslateMath(math, template, lang) {
  ...
  var maybeMathTranslations = [// multiplication sign as a centered dot
  {
...
function getOrderedPairRegexString(lang) {
  ...
  var numberAndOrLetter = frac + "|" + variable + "|(?:" + decimal + "|" + integer + ")(?:" + variable + ")?"; // NOTE(danielhollas): We allow comma and semicolon for all langs
...
function getSeparator(template, regex, lang) {
  var sepDefault = ','; // Languages with decimal comma typically use semicolon

  if (MATH_RULES_LOCALES.DECIMAL_COMMA.includes(lang)) {
    sepDefault = ';';
  }

  var match; // Return separator from the template if detected
...
function wrapParens(regex, left, right, capture) {
  if (capture === void 0) {
    capture = false;
  }
...
function translateIntervals(math, template, lang) {
  ...
  var rightClosedInterval = new RegExp(wrapParens(intervalUS, '(', ']', true), 'g'); // Detect range separator in the template, can be comma or semicolon
  ...
  var intervalTranslations = [// open intervals with inverted brackets
  {
    ...
    langs: MATH_RULES_LOCALES.OPEN_INT_AS_BRACKETS,
    regex: closedInterval,
    replace: "$1[$2" + sep + "$4$5]"
  }, {
    langs: MATH_RULES_LOCALES.OPEN_INT_AS_BRACKETS,
    regex: leftClosedInterval,
    replace: "$1[$2" + sep + "$4$5["
  }, {
    langs: MATH_RULES_LOCALES.OPEN_INT_AS_BRACKETS,
    regex: rightClosedInterval,
    replace: "$1]$2" + sep + "$4$5]"
...
function translateCoordinatesOrOpenIntervals(math, template, lang) {
  ...
  var coordsOrOpenIntervalUS = new RegExp(wrapParens(orderedPairUS, '(', ')', true), 'g'); // First look into the English string
  ...
  var sep = match[3];
  var rightParen = match[5]; // Verify that left and right parentheses|brackets make sense

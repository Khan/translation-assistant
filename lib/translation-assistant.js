...
function stringToGroupKey(str) {
    ...
    var texts = maths.map(function (math) {
        ...
        allMatches(math, /\\text{([^}]*)}/g, function (matches) {
    ...
    str = str.replace(MATH_REGEX, '__MATH__').replace(GRAPHIE_REGEX, '__GRAPHIE__').replace(WIDGET_REGEX, '__WIDGET__').replace(/__MATH__[\t ]*__WIDGET__/g, '__MATH__ __WIDGET__').replace(BOLD_REGEX, function (match) {
...
function getMapping(englishStr, translatedStr, lang, findRegex, mathDictionary) {
    ...
    if (findRegex === MATH_REGEX) {
        inputs = inputs.map(function (input) {
            var result = input;
            for (var _iterator = Object.entries(mathDictionary), _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
                var _ref;
...
function getMathDictionary(englishStr, translatedStr) {
    ...
    inputs.forEach(function (input) {
        var normalized = input.replace(TEXT_REGEX, '__TEXT__');
    ...
    outputs.forEach(function (output) {
        var normalized = output.replace(TEXT_REGEX, '__TEXT__');
    ...
    Object.keys(inputMap).forEach(function (key) {
        if (/__TEXT__/.test(key)) {
            var _ret = (function () {
                ...
                var inputTexts = {};
                allMatches(input, /\\text{([^}]*)}/g, function (matches) {
                ...
                var outputTexts = {};
                allMatches(output, /\\text{([^}]*)}/g, function (matches) {
...
function createTemplate(englishStr, translatedStr, lang) {
    ...
    var mathDictionary = getMathDictionary(englishStr, translatedStr);
    ...
    try {
        return {
            ...
            mathMapping: getMapping(englishStr, translatedStr, lang, MATH_REGEX, mathDictionary),
...
function populateTemplate(template, englishStr, lang) {
    ...
    maths = maths.map(function (math) {
        ...
        for (var _iterator2 = Object.entries(dict), _isArray2 = Array.isArray(_iterator2), _i2 = 0, _iterator2 = _isArray2 ? _iterator2 : _iterator2[Symbol.iterator]();;) {
            var _ref2;
            ...
            var englishText = _ref2[0];
            var translatedText = _ref2[1];
...
var TranslationAssistant = (function () {
    ...
    TranslationAssistant.prototype.getSuggestionGroups = function getSuggestionGroups(items) {
        ...
        items.forEach(function (obj) {
            var key = stringToGroupKey(_this2.getEnglishStr(obj));

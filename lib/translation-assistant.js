...
var _require = require('./math-translator'),
    translateMath = _require.translateMath,
    normalizeTranslatedMath = _require.normalizeTranslatedMath; // Matches math delimited by $, e.g.
// $x^2 + 2x + 1 = 0$
// $\text{cost} = \$4$


var MATH_REGEX = /\$(\\\$|[^$])+\$/g; // Matches graphie strings,
// e.g. ![](web+graphie://ka-perseus-graphie.s3.amazonaws.com/542f2b4e297910eed545a5c29c3866918655bab4)

var GRAPHIE_REGEX = /!\[\]\([^)]+\)/g; // Matches pure image and graphie link strings,
...
var IMAGE_REGEX = /https:[^\s]+\.png|web\+graphie:[a-z0-9.\-/]+(?=[\s,]|$)/g; // Matches widget strings, e.g. [[☃ Expression 1]]

var WIDGET_REGEX = /\[\[[\u2603][^\]]+\]\]/g; // TODO(michaelpolyak): Add support for other \text commands:
...
var TEXTBF_REGEX = /\\textbf\s*{([^}]*)}/g; // Use two line feeds to split lines because this is how Markdown delineates
// paragraphs.
...
function stringToGroupKey(str) {
  var maths = str.match(MATH_REGEX) || []; // This maps formula to an array which may contain 0 or more
  ...
  var texts = maths.map(function (math) {
    ...
    allMatches(math, regex, function (matches) {
      return result.push( // TEXT_REGEX capture group is at index 1 and TEXTBF_REGEX at
...
function getMapping(englishStr, translatedStr, lang, findRegex, // TODO(kevinb): change mathDictionary to mathDictionaries
mathDictionary) {
...
function getMathDictionary(englishStr, translatedStr, lang) {
  ...
  var outputs = translatedStr.match(MATH_REGEX) || []; // Used in maybeTranslateMath() to pattern-match the translated math
  // from the entire template
  ...
  outputs.forEach(function (output) {
    ...
    replaceRegexes.forEach(function (_ref2) {
      var regex = _ref2[0],
          str = _ref2[1];
      normalized = normalized.replace(regex, str);
    });
  ...
  Object.keys(inputMap).forEach(function (key) {
    matchRegexes.forEach(function (_ref3) {
      var match = _ref3[0],
          regex = _ref3[1];
      ...
      if (match.test(key)) {
        ...
        var output = outputMap[key]; // Compute the set of all natural language text within \text{}
        ...
        var outputKeys = Object.keys(outputTexts); // We assume that the order of \text{} and \textbf{} blocks will
...
function replaceTextInMath(englishMath, dict) {
  ...
  var _loop = function _loop() {
    var _Object$entries$_i = _Object$entries[_i],
        englishText = _Object$entries$_i[0],
        translatedText = _Object$entries$_i[1];
    textCommands.forEach(function (cmd) {
      var escapedEnglishText = escapeForRegex(englishText);
      var regex = new RegExp("\\\\" + cmd + "(\\s*){" + escapedEnglishText + "}", 'g'); // make sure the spacing matches in the replacement
...
function populateTemplate(template, englishStr, lang) {
  ...
  if (template.mathMapping.englishToTranslated.length) {
    ...
    // string.
    var englishDictionary = getMathDictionary(englishStr, englishStr, lang);
    var englishMapping; // This call to getMapping() should be safe because we're
  ...
  var widgets = englishStr.match(WIDGET_REGEX) || []; // Used in maybeTranslateMath() (called in translateMath)
...
var TranslationAssistant = /*#__PURE__*/function () {
  /**
  ...
  var _proto = TranslationAssistant.prototype;

  _proto.suggest = function suggest(itemsToTranslate) {
    var _this = this;
    ...
    return itemsToTranslate.map(function (item) {
      ...
      if (Object.prototype.hasOwnProperty.call(suggestionGroups, normalStr)) {
        var template = suggestionGroups[normalStr].template; // This error is probably due to math being different between
  ...
  _proto.getSuggestionGroups = function getSuggestionGroups(items) {
    var _this2 = this;
